...
class ECGAnalyzer {
  ...
  async handleFileUpload(event) {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/ecg/upload", {
        method: "POST",
        body: formData,
      });
...
async updatePolarDisplay() {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:5000/api/ecg/polar-data/${this.polarMode}?current_time=${this.currentAnimationTime}`);
...
mapLeadNameToBackend(frontendLeadName) {
  ...
  async classifyECG() {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/ecg/classify", {
        method: "POST",
        headers: {
