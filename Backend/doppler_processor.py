...
class DopplerSoundGenerator:
    ...
    def generate_vehicle_sound(self, base_frequency=120, velocity=30, sampling_rate=3000):
        """
        ...
        """
        try:
            # Generate at the target sampling rate directly
            total_samples = int(self.duration * sampling_rate)
            time_array = np.linspace(0, self.duration, total_samples)
            ...
            # Generate phase with proper cumulative sum to avoid large values
            phase = 2 * np.pi * np.cumsum(doppler_frequency) / sampling_rate
            
            # Generate realistic engine sound with multiple harmonics
            # Higher harmonics will alias at low sampling rates
            engine_sound = (
                0.6 * np.sin(phase) +  # Fundamental
                0.3 * np.sin(2 * phase + 0.5) +  # 2nd harmonic
                0.2 * np.sin(3 * phase + 1.2) +  # 3rd harmonic  
                0.1 * np.sin(4 * phase + 0.8) +  # 4th harmonic
                0.05 * np.sin(6 * phase + 2.1)   # 6th harmonic
            )
            
            # Add engine rumble (low frequency modulation)
            rumble_freq = 2.0  # Hz - engine rumble frequency
            rumble = 0.1 * np.sin(2 * np.pi * rumble_freq * time_array)
            engine_sound *= (1 + rumble)
            
            # Add broadband noise for realism (engine noise)
            broadband_noise = 0.15 * np.random.randn(len(time_array))
            ...
            # Remove DC offset and ensure finite values
            ...
            # ENHANCED NORMALIZATION - Better distortion prevention
            audio_max = np.max(np.abs(combined_audio))
            if audio_max > 0:
                # Use more conservative normalization
                combined_audio = combined_audio / audio_max * 0.7  # Reduced from 0.8 to 0.7 for more headroom
            else:
                # If audio is all zeros, create a simple tone
                combined_audio = 0.3 * np.sin(2 * np.pi * base_frequency * time_array)  # Reduced amplitude
            
            # ADDITIONAL: Apply soft clipping to prevent hard distortion
            ...
            combined_audio = soft_clip(combined_audio)
            
            # Final sanity check with tighter bounds
            combined_audio = np.clip(combined_audio, -0.95, 0.95)
            combined_audio = combined_audio.astype(np.float32)

            return time_array, combined_audio
            
        except Exception as error:
            logger.error(f"Sound generation error: {error}")
            # If anything fails, generate a simple sine wave that always works
            total_samples = int(self.duration * sampling_rate)
            time_array = np.linspace(0, self.duration, total_samples)
            simple_audio = 0.3 * np.sin(2 * np.pi * base_frequency * time_array)  # Reduced amplitude
            return time_array, simple_audio.astype(np.float32)
...
class VehicleDopplerAnalyzer:
    ...
    def analyze_audio_signal(self, audio_file_path):
        ...
        # Generate waveform visualization data with aliasing detection
    ...
    def _basic_audio_analysis(self, audio_signal, sample_rate, reason=""):
        """Basic analysis using audio characteristics"""
    ...
    def _generate_waveform_visualization(self, audio_signal, sample_rate, analysis_sample_rate=None):
        """Generate waveform data for frontend visualization with aliasing detection"""
        signal_duration = len(audio_signal) / sample_rate
        ...
        # Calculate if aliasing is likely based on spectral content
        is_aliasing = False
        nyquist_frequency = sample_rate / 2
        
        # Analyze spectral content to detect potential aliasing
        try:
            # Compute FFT to find maximum frequency content
            fft_result = np.fft.fft(audio_signal)
            frequencies = np.fft.fftfreq(len(fft_result), 1/sample_rate)
            magnitude = np.abs(fft_result)
            
            # Find significant frequency components
            significant_freqs = frequencies[magnitude > 0.1 * np.max(magnitude)]
            if len(significant_freqs) > 0:
                max_detected_freq = np.max(np.abs(significant_freqs))
                # If significant content near or above Nyquist, mark as aliasing
                if max_detected_freq > nyquist_frequency * 0.8:
                    is_aliasing = True
        except:
            pass
